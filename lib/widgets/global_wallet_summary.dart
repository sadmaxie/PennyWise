          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(
                    tx.isIncome ? Icons.arrow_circle_down_outlined : Icons.arrow_circle_up_outlined,
                    size: 25,
              ...
              Row(
                ...
                children: [
                  ...
                  Text(DateFormat.yMMMd().format(tx.date), style: const TextStyle(color: Colors.white70)),
                ],
              ),
            ],
          ),
      ...
      children: goals.map((goal) {
        ...
        return Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(goal.name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              const SizedBox(height: 6),
              Text('Missing: \$${missing.toStringAsFixed(2)}', style: const TextStyle(color: Colors.amber)),
              const SizedBox(height: 4),
              Text('Goal: \$${total.toStringAsFixed(2)}', style: const TextStyle(color: Colors.white70)),
            ],
          ),
        );
      }).toList(),
...
class TransactionHistoryList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final transactions = provider.allTransactions..sort((a, b) => b.date.compareTo(a.date));
...
class GoalWalletList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final provider = Provider.of<WalletProvider>(context);
    final goals = provider.goalWallets..sort((a, b) {
          ...
          child: Row(
            ...
            children: [
              CircleAvatar(
                ...
                backgroundImage: hasImage ? FileImage(File(wallet.imagePath!)) : null,
                child: !hasImage ? const Icon(Icons.flag_outlined, color: Colors.white) : null,
              ),
              const SizedBox(width: 16),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(wallet.name, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                    const SizedBox(height: 8),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text("\$${current.toStringAsFixed(2)}", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.white)),
                        Text("of \$${total.toStringAsFixed(2)}", style: const TextStyle(fontSize: 14, color: Colors.white70)),
                      ],
                    ),
                    const SizedBox(height: 8),
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text("${(progress * 100).toStringAsFixed(0)}% Reached", style: const TextStyle(fontSize: 12, color: Colors.white54)),
                        Text("\$${amountLeft.toStringAsFixed(2)} left", style: const TextStyle(fontSize: 12, color: Colors.white54)),
                      ],
                    ),
                  ],
                ),
